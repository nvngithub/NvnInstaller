...
namespace NvnInstaller.MsiDotNet {
    ...
    sealed internal class MsiInterop {
        ...
        extern static public MsiError MsiEnumFeatures(string product, uint index,
          StringBuilder feature, StringBuilder featureParent);
...
namespace NvnInstaller.MsiDotNet {
    ...
    internal class MsiConnection : IDisposable {
        ...
        public void Open() {
            ...
            uint res = MsiDbInterop.MsiOpenDatabase(_fileName, 0, out _database);
            if (res != 0) {
        ...
        public void Close() {
            ...
            if (_database != IntPtr.Zero) {
                MsiDbInterop.MsiCloseHandle(_database);
    ...
    internal class MsiCommand : IDisposable {
        ...
        public MsiDataReader ExecuteReader() {
            ...
            uint res = MsiDbInterop.MsiDatabaseOpenView(_conn.Handle, _command, out _view);
            if (res != 0) {
        ...
        private void Dispose(bool disposing) {
            if (!_disposed && _view != IntPtr.Zero) {
                MsiDbInterop.MsiCloseHandle(_view);
                _view = IntPtr.Zero;
                _disposed = true;
            }
        }
    ...
    internal class MsiDataReader : IDisposable {
        ...
        public bool Read() {
            ...
            if (_record == IntPtr.Zero) {
                res = MsiDbInterop.MsiViewExecute(_command.Handle, IntPtr.Zero);
                if (res != 0) {
                    throw new Exception("Failed to run command. Error code = " + res);
                }
            } else {
                MsiDbInterop.MsiCloseHandle(_record);
            }

            res = MsiDbInterop.MsiViewFetch(_command.Handle, out _record);
            if (res == 259) {
                return false;
            }
        ...
        public string GetString(uint column) {
            ...
            uint res = MsiDbInterop.MsiRecordGetString(_record, column, sb, ref len);
            if (res == 234) {
                sb.Capacity = len++;
                res = MsiDbInterop.MsiRecordGetString(_record, column, sb, ref len);
            }
        ...
        public int GetInteger(uint column) {
            ...
            int res = MsiDbInterop.MsiRecordGetInteger(_record, column);
    ...
    internal static class MsiDbInterop {
